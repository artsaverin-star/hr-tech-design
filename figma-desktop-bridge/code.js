...
globalThis.hrtechVersion = '1.5.1';
...
globalThis.hrtechDiff = async function (srcId, dstId) {
  ...
  async function bag(id) {
    ...
    const out = new Set();
    if (!n) return out;
    const all = n.findAllWithCriteria({ types: ['TEXT'] });
    for (let i = 0; i < Math.min(all.length, 5000); i++) {
      const t = all[i];
      if (!t.visible) continue;
      const ch = norm(t.characters);
      if (ch.length > 2) out.add(ch);
    }
    return out;
  }
  const src = await bag(srcId), dst = await bag(dstId);
  const fabricated = [];
  for (const d of dst) {
    if (src.has(d)) continue;
    let partial = false;
    for (const s of src) { if (s.includes(d) || d.includes(s)) { partial = true; break; } }
    if (!partial) fabricated.push(d.slice(0, 80));
  }
  const missing = [];
  for (const s of src) {
    if (dst.has(s)) continue;
    let partial = false;
    for (const d of dst) { if (d.includes(s) || s.includes(d)) { partial = true; break; } }
    if (!partial) missing.push(s.slice(0, 80));
  }
  return { fabricated: fabricated.slice(0, 40), missing: missing.slice(0, 40), srcCount: src.size, dstCount: dst.size };
};
globalThis.hrtechScan = async function (nodeId, opts) {
  opts = opts || {};
  const maxTexts = opts.maxTexts || 400;
  figma.skipInvisibleInstanceChildren = true;
  const node = await figma.getNodeByIdAsync(nodeId);
  if (!node) return { error: 'node not found: ' + nodeId };
  const x0 = node.absoluteTransform[0][2], y0 = node.absoluteTransform[1][2];
  const all = node.findAllWithCriteria({ types: ['TEXT'] });
  const texts = [];
  const iterCap = Math.min(all.length, 5000);
  for (let i = 0; i < iterCap && texts.length < maxTexts; i++) {
    const t = all[i];
    if (!t.visible) continue;
    ...
    texts.push({
      id: t.id,
      c: ch.length > 140 ? ch.slice(0, 140) + '\u2026' : ch,
      x: Math.round(t.absoluteTransform[0][2] - x0),
      y: Math.round(t.absoluteTransform[1][2] - y0),
      s: t.fontSize === figma.mixed ? 0 : t.fontSize,
    });
  }
  const zones = 'children' in node ? node.children.slice(0, 60).map((ch2) => ({
    id: ch2.id, name: ch2.name.slice(0, 40), type: ch2.type,
    x: Math.round(ch2.x), y: Math.round(ch2.y), w: Math.round(ch2.width), h: Math.round(ch2.height),
    visible: ch2.visible,
  })) : [];
...
figma.ui.onmessage = async (msg) => {
  ...
  if (msg.type === 'HRTECH_MODEL') {
    figma.root.setSharedPluginData('hrtech', 'model', msg.model || 'auto');
    figma.notify('HR TECH \u00b7 Model: ' + (msg.model || 'sonnet'));
    return;
  }
  ...
  "desktop-to-mobile": [
      ...
      "FRAME — the ONLY thing taken from any reference: 375 px wide, height hugs content, side margins 20 (content width 335). Everything else is assembled from DS components: 'System / Status Bar / iPhone' (54) on top, 'Home Indicator' (21) at the bottom.",
      ...
      "ZONE MAP · content by type: table → grouped list of '💠 List-Item' (group rows by the date/category column into divider rows; statuses → '❇️ Tag · Status' or caption text); cards → stacked full-width cards; detail-article → sections on the 28/20/16/14 scale, long collapsible blocks → '💠 Item → Accordion' (content swapped into ↳ Slot as a local 🔁 component); form → stacked '❇️ Input'/'❇️ Date Input' fields; chat → user message as right-aligned grey bubble, assistant reply with avatar+name and PRESERVED list/bold structure, actions row + sources chip, composer ('❇️ Input' + send '❇️ Button' Variant=Icon) pinned above Home Indicator, disclaimer Caption/M below; dashboard → widgets stacked in one column. Aside/inspector panels do not fit on mobile — report them as a candidate separate screen instead of cramming them in.",
