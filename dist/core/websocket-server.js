...
export class FigmaWebSocketServer extends EventEmitter {
        ...
        if (message.type) {
            ...
            if (message.type === 'HRTECH_HOST_STATUS_REQUEST') {
                this.collectHrtechHostStatus()
                    .then((data) => { try {
                    ws.send(JSON.stringify({ type: 'HRTECH_HOST_STATUS', data }));
                }
                catch { } })
                    .catch(() => { });
                return;
    ...
    async collectHrtechHostStatus() {
        const out = { account: null, usage: null, platform: process.platform };
        try {
            ...
            if (acc)
                out.account = { email: acc.emailAddress || null, org: acc.organizationName || null };
        }
        ...
        if (process.platform === 'darwin') {
            try {
                ...
                if (token) {
                    ...
                    if (resp.ok)
                        out.usage = await resp.json();
                    else
                        out.usageError = 'HTTP ' + resp.status;
                }
            }
            catch (e) {
                out.usageError = e instanceof Error ? e.message.slice(0, 120) : 'keychain error';
            }
        }
        return out;
    }
