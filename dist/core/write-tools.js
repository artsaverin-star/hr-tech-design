...
Layers: If your code creates helper frames, placeholder nodes, or intermediate layers that aren't part of the final result, remove them.`, {
        ...
        try {
            ...
            const result = await connector.executeCodeViaUI(code, Math.min(timeout, 300000));
            return {
                content: [
                    {
                        type: "text",
...
return {
  ...
  results
            ...
            const result = await connector.executeCodeViaUI(script, Math.min(timeout, 300000));
            if (result.error) {
                return {
                    content: [
                        {
