...
name: hrtech-arcadia
description: >-
  Когда нужно узнать, как что-то устроено НА САМОМ ДЕЛЕ в живом сервисе HR Tech — «как это сделано на проде»,
  «найди исходники», «какой там текст на самом деле», «прототип должен совпадать с продом», «какие там состояния»,
  «какие отступы», «какой это компонент в коде», «где лежит этот сервис», а также перед тем как придумывать тексты,
  состояния или размеры для продукта, который УЖЕ СУЩЕСТВУЕТ. Учит искать в Аркадии: карта фронтендов, кодпоиск,
  тексты, токены, эталонные скриншоты, соответствие HRDS↔React, оболочка интранета. НЕ для сборки борда
  (это hrtech-spec) и не для правки ДС-компонентов (это hrtech-audit).
---
