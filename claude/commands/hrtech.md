...
0d. **Layout-sizing gotchas (CRITICAL — these cause silent visual bugs):**
   ...
   - **Tag · Status** leading icon lives in the nested `Slot ←` instance's `← Icon#616:5` (NOT the tag's own `← Icon#29143:237`); empty = hollow ring. Its color variant prop is NOT named `Style` — `Style=Blue/Gray` silently falls to the default (red); inspect the real variant prop names first.
   - **Orphan frame:** `createFrame()` auto-parents to `currentPage`; if you forget to `appendChild` a built card into your tree it silently floats at (0,0) named `Frame` and the block renders empty (no error). Always append containers (a `card(w,parent)` factory that appends inside is safest). Sweep stragglers: remove top-level FRAMEs named `Frame`.
   - **Checkbox:** pick the variant with BOTH `Selected=on|off` AND `Mixed=off` at creation, else you get the mixed `−` glyph; flipping via `setProperties` after instantiating the wrong variant can silently no-op.
