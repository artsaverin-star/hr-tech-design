...
- **Время у коллеги — на ОБЕИХ платформах.** В десктопном поповере это ПЕРВАЯ строка, над списком
  номеров (стиль Caption 12, secondary — тот же, что у подписей «Основной»/«Офисный»; текст-узел
  «Время у коллеги», `layoutAlign=STRETCH`). Оно про человека, а не про номер, поэтому строка одна
  на весь поповер, а не в каждой строке списка. Формулировка совпадает с мобильной дословно.
...
- **Единый каркас мобильной шторки (сведён 19.08 по вопросам разработки).** Все состояния — один
  скелет, различается ТОЛЬКО набор кнопок:
  `pull-bar` → **Title = имя номера** («Основной номер», «Офисный») → **Subtitle строка 1** = сам номер
  с пометками («+7 925 897-30-40 · Только СМС») **или** «Номер скрыт» → **Subtitle строка 2**
  = **время у коллеги** («Время у коллеги: 12:20, Jerusalem») → **Subtitle строка 3** = одна и та же
  фраза везде: «Вам позвонит Яндекс и соединит с коллегой» → кнопки в
  порядке «Позвонить через Яндекс» (primary, всегда) · «Позвонить напрямую» (только если номер открыт
  и набираемый) · «Выбрать другой номер» (только если номеров больше одного).
- **ТЕКСТЫ В ЭТОЙ ФИЧЕ ВЫВЕРЕНЫ РЕДАКТОРОМ — своих не писать.** Сведение к единому виду делается
  ВЫБОРОМ одной из уже существующих в макете строк и её ПЕРЕСТАНОВКОЙ, а не переписыванием. Если
  на разных экранах живут две редакторские формулировки одного смысла (было: «Вам позвонит Яндекс и
  соединит с коллегой» vs «При звонке через Яндекс сначала поступит звонок вам, затем — соединение»),
  выбор из них — вопрос к редактору, а не к дизайнеру. Правило общее для всей фичи, не только для
  шторок. См. [[use-only-ds-components]] — та же логика, что с компонентами.
- **Время у коллеги — обязательная строка, а не украшение.** Оно решает, звонить ли вообще, поэтому
  стоит ДО кнопок и есть на всех шторках с номером — в том числе на «Номер скрыт» (время принадлежит
  человеку, а не номеру). Не показываем там, где звонить некуда или уже звоним («Нет номера»,
  «Соединяем»). В нерабочее время у коллеги строку выделяем акцентом, а не прячем — тихое «3:20»
  тем же серым читается как обычная справка.
- **Заголовок = состояние ТОЛЬКО там, где звонить некуда или уже звоним:** «Соединяем», «Нет номера».
  Во всех остальных шторках заголовок отвечает «куда звоним», а «Номер скрыт» — это ЗНАЧЕНИЕ вместо
  цифр (так же устроен десктопный поповер: Caption «Основной» / Label «Номер скрыт»).
- **Отсчёт.** Счётчик живёт в подписи («Вам позвонит Яндекс через 5»), кнопка — всегда «Отменить»
  без цифры. Цифра на кнопке тикает каждую секунду и читается наоборот («отменится через 5»);
  редакторская строка без единицы измерения ещё и снимает склонение («через 1 секунду / 2 секунды»).
  На отсчёте кнопка ОДНА — ни «Позвонить напрямую», ни «Выбрать другой номер»: два параллельных
  дозвона одновременно не запускаем.
- **Крестика в шапке нет ни в одном состоянии.** Закрытие — pull-bar/свайп/тап по фону. На отсчёте ✕
  дополнительно двусмыслен: «закрыть» прочитается как «отменить звонок».
...
### Фильтры на мобилке — кнопка → шторка → категория (BLUEPRINT)
Мобильная замена инлайн-чипов/аккордеона «Фильтры» (десктоп-паттерн Filter не влезает на 375):
- **Триггер.** В строке под заголовком: `💠 Input · Search` (fill) + справа **иконка-кнопка «Фильтры»** = `❇️ Button` **Variant=Icon**, Size M, View как у хедер-кнопок, глиф — фильтр/ползунки (при активных фильтрах — счётчик-бейдж). Инлайн-чипов и аккордеона «Фильтры» на мобилке НЕТ. Обёртка-ряд: HORIZONTAL, padding 16, itemSpacing 8, поиск `layoutGrow=1`, кнопка 40×40 `layoutGrow=0`.
- **Шторка «Фильтры»** = `❇️ Drawer [mobile]` `4d65c84f128ab9a2bcb6f99320dcda3f729926e1`, **по высоте контента** (bottom-sheet, БЕЗ пустоты — не растягивать на весь экран): заголовок «Фильтры» + ✕; тело — категории строками `💠 List-Item` `a7da95f59b50ac222941653cc6c6cb2ac8b9fb0f` (Участники, Теги, Дата, Тип участия), у каждой Title слева + **Trailing-значение** (текущий выбор: «Алина Батталова, +3» / «2 выбрано» / «Сен — Дек 2025» / «Все») + **шеврон** `Arrow : Short : Right` `479ba1a8224407eba4c2d2eb6bf2f91cfd8ad570` в After-слоте (drill-down; НЕ чекбокс). Поиска в шторке нет. Футер: **Сбросить** (ghost) + **Применить** (primary).
- **Категория открывается ОДНИМ ИЗ ДВУХ способов (общий паттерн «Назад vs Поверх», задокументирован отдельным гайдом):**
  - **Способ 1 · Навигация ВНУТРИ шторки (рекомендуется, дефолт).** Для простых категорий (список, тумблеры, дата). Тап → контент той же шторки уезжает на «Участники» с навбаром **‹ Фильтры** сверху; шторка **плавно ресайзится под контент** (низ на месте, верх едет вверх). Движение есть — «без прыжков» неточно; полного нуля движения между коротким и длинным списком не бывает. Одна шторка, без второй. «Назад» → «Фильтры».
  - **Способ 2 · Шторка ПОВЕРХ (стек).** Для тяжёлой самодостаточной настройки (конструктор фильтра, много под-опций). Тап → отдельный `Drawer [mobile]` поверх; «Фильтры» **отъезжает назад уменьшенной затемнённой карточкой** (rescale ~0.94, инсет по бокам, к верху, scrim ~0.4) и выглядывает сверху — стопкой карточек. «Назад/Отмена» → «Фильтры».
  - **АНТИПАТТЕРН:** НЕ делать «Фильтры» высокой полупустой шторкой ради того, чтобы она «выглядывала» под стеком — пустота выглядит сломанной, а короткая шторка, задранная наверх, читается как телепорт. Либо навигация внутри (способ 1), либо честный iOS-стек с отъезжающей карточкой (способ 2).
- **Правила.** Каждый уровень — bottom-sheet по контенту. Счётчик активных фильтров на кнопке-триггере. Значения категорий — живой current-state, не заглушки. Реф-сборка + документация паттерна (описаны оба способа + «когда какой»): файл «Мобильная версия v2» Section 3 — шторка `15827:1815511`, способ 1 `15828:1841112`, способ 2 `15828:1845288`, док-фрейм `📐 Паттерн · Фильтры` `15827:1826727`, триггер во Frame 22 (`m-search-row`).
